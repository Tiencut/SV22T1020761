...
using SV22T1020218.Models.Security;
using System;
using System.Linq;
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(string username, string password)
        {
            try
            {
                ...
                // Kiểm tra username và password đã mã hoá với dữ liệu trong database
                ...
                // 1. Tạo dữ liệu người dùng để lưu vào giấy chứng nhận (ClientPrincipal)
                var userData = new WebUserData
                {
                    ...
                    // FIX LỖI: Kiểm tra RoleNames có bị Null hoặc rỗng không trước khi Split
                    Roles = string.IsNullOrEmpty(userAccount.RoleNames)
                            ? new System.Collections.Generic.List<string>()
                            : userAccount.RoleNames.Split(',').Select(r => r.Trim()).ToList()
                };

                // 2. Tạo giấy chứng nhận (ClientPrincipal) cho người dùng
                var principal = userData.CreatePrincipal();

                // 3. Cấp giấy chứng nhận cho người dùng ghi nhận phiên đăng nhập
            ...
            {
                // Bắt lỗi nếu mất kết nối Database hoặc lỗi code nội bộ
