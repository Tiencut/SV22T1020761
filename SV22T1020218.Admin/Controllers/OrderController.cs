...
using SV22T1020218.Admin; // Dùng cho WebUserRoles
using System;
using System.Linq;
...
namespace SV22T1020218.Admin.Controllers
{
    ...
    public class OrderController : Controller
    {
            ...
            try
            {
                ...
                if (input == null)
                    input = new OrderSearchInput
                    {
                        ...
                        SearchValue = "",
                        Status = 0,
                        DateFrom = null, // Chỗ này nên để null thay vì DateTime.Now để mặc định tìm tất cả
                        DateTo = null,
                    };
                return View(input);
            }
        ...
        public IActionResult UpdateCartItem(int productId, int quantity, decimal salePrice)
        {
            try
            {
                // Đã xóa cái if (Request.Method == "POST") thừa ở đây
                if (quantity <= 0)
                    return Json(new ApiResult(0, "Số lượng phải lớn hơn 0"));
                if (salePrice < 0)
                    return Json(new ApiResult(0, "Giá bán không hợp lệ!"));
        ...
        public async Task<IActionResult> CreateOrder(int customerID = 0, string province = "", string address = "")
        {
            try
            {
                ...
                ShoppingCartService.ClearCart();
                return Json(new ApiResult(orderID)); // Trả về ID đơn hàng thành công
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> UpdateDetail(int orderId, int productId, int quantity, decimal salePrice)
        {
            try
            {
                // Đã gỡ bỏ cái if (Request.Method == "POST") thừa thãi
                if (quantity <= 0)
                    return Json(new ApiResult(0, "Số lượng phải lớn hơn 0"));
                ...
                if (result)
                {
                    return Json(new ApiResult(1)); // Thành công
                }
                else
                {
        ...
        public async Task<IActionResult> DeleteDetail(int id = 0, int productId = 0)
        {
            try
            {
                ...
                return View(model); // Hoặc PartialView tùy cấu trúc của ông cháu
            }
            catch (Exception ex)
            {
                return Content($"<div class='alert alert-danger'>Lỗi: {ex.Message}</div>");
